...
% function [freq_final,freq_error,speed,diffusivity,diffusivity_err,tau,peak_fwhm,SNR_fft] = TGSPhaseAnalysis(pos_file,neg_file,grat,start_phase,two_mode,rangefrac,overlay1,overlay2,time_index)
% function [freq_final,freq_error,speed,diffusivity,diffusivity_err,tau,peak_fwhm,SNR_fft] = TGSPhaseAnalysis(pos_file,neg_file,grat,start_phase,two_mode,rangefrac,overlay1,overlay2)


function [freq_final,freq_error,speed,diffusivity,diffusivity_err,tau, tauErr, paramrA, AErr, ParamBeta, BetaErr, paramB, BErr, paramTheta, thetaErr, paramC, CErr] = TGSPhaseAnalysis(pos_file,neg_file,grat,start_phase,two_mode,overlay1,overlay2,baselineBool,POSbaselineStr, NEGbaselineStr)
...
%           display(f2.beta)
            ...
            if plot_final
                ...
                %%%%%%%%%%%%%%%
                %Angus' added piece-------------------------------------------------------------------------------------------------------------------
                %would like to extract the coefficients of the fitted curve for each of the runs as well as their lower and upper bounds
%                 if print_parameters
%                     display(strcat('For this run:   ',pos_file,' there are the following fitted parameters for the TGS equation:'));
%                     fprintf('\n Parameters:   A= %.8g α= %.8g q= %.8g β= %.8g B= %.8g f= %.8g θ= %.8g τ= %.8g C= %.8g\n',f2.A, f2.k, q, f2.beta, f2.B, peak_freq, f2.p, f2.t, f2.D);
%                     fprintf('Lower bounds (95 percent confidence)   ALB= %.8g αLB= %.8g βLB= %.8g BLB= %.8g θLB= %.8g τLB= %.8g CLB= %.8g\n',error(1,1), error(1,2), error(1,3), error(1,4), error(1,5), error(1,6), error(1,7));
%                     fprintf('Upper bounds (95 percent confidence)   AUB= %.8g αUB= %.8g βUB= %.8g BUB= %.8g θUB= %.8g τUB= %.8g CUB= %.8g\n\n',error(2,1), error(2,2), error(2,3), error(2,4), error(2,5), error(2,6), error(2,7));
%                     fprintf('Fit start time = %.8g\nFit start index = %.8g\n\n', start_time2, start_index2);
%                     
%                     %calculate errors similarly to thermal diff. to be printed
%                     errorA = (f2.A - error(1,1))/2;
%                     errorBeta = (f2.beta - error(1,3))/2;
%                     errorB = (f2.B - error(1,4))/2;
%                     errorTheta = (f2.p - error(1,5))/2;
%                     errorTau = (f2.t - error(1,6))/2;
%                     errorC = (f2.D - error(1,7))/2;
%                     
%                     printFile1 = 'C:\Users\apcwy\Dropbox (The University of Manchester)\Physics PhD\MIT_2022\TGS_Data\Iron_Peak_Skipping\start_phase_4\ParametersAndErrorsstart_phase_4.txt';
%                     fid1 = fopen( printFile1, 'a' );
%                     fprintf(fid1, pos_file + string(' %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g %.8g\n'), f2.A, errorA, diffusivity, diffusivity_err, f2.beta, errorBeta, f2.B, errorB, f2.p, errorTheta,f2.t,errorTau,f2.D,errorC);
%                     fclose(fid1);
%                 end
                %Angus' added piece ends --------------------------------------------------------------------------------------------------------------
                figure()
